...
import com.jiebao.platfrom.common.annotation.Limit;
...
public class LoginController {
    ...
    public JiebaoResponse login(
        ...
        if (user == null)
            throw new JiebaoException(errorMessage);
        if (!StringUtils.equals(user.getPassword(), password))
            throw new JiebaoException(errorMessage);
        if (User.STATUS_LOCK.equals(user.getStatus()))
            throw new JiebaoException("账号已被锁定,请联系管理员！");
        ...
        return new JiebaoResponse().message("认证成功").data(userInfo);
    }

    @GetMapping("index/{username}")
    public JiebaoResponse index(@NotBlank(message = "{required}") @PathVariable String username) {
        ...
        return new JiebaoResponse().data(data);
    }

    @RequiresPermissions("user:online")
    @GetMapping("online")
    public JiebaoResponse userOnline(String username) throws Exception {
        ...
        return new JiebaoResponse().data(activeUsers);
    }
    ...
    public void kickout(@NotBlank(message = "{required}") @PathVariable String id) throws Exception {
        ...
        if (kickoutUser != null && StringUtils.isNotBlank(kickoutUserString)) {
            ...
            // 删除对应的 token缓存
            redisService.del(JiebaoConstant.TOKEN_CACHE_PREFIX + kickoutUser.getToken() + "." + kickoutUser.getIp());
        }
    }

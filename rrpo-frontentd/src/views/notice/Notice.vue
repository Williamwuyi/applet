<template>
  <a-card :bordered="false" class="card-area">
    <!--面包屑-->
    <a-breadcrumb class="bre">
      <a-breadcrumb-item><span @click="onClickTop">主页</span></a-breadcrumb-item>
      <a-breadcrumb-item><span>通知公告</span></a-breadcrumb-item>
    </a-breadcrumb>
    ...
    <a-dropdown >
          <a-menu slot="overlay">
            <a-menu-item key="export-data" @click="exprotExccel" v-hasPermission="'notice:export'">打印</a-menu-item>
          </a-menu>
          <a-button>
    ...
    >
        <a slot="name" slot-scope="text,record" style="color:#6290FF" @click="look(record)">{{ text }}</a>
      ...
      <template slot="operation" slot-scope="text, record">
        <a-icon v-if="record.status !== '3'" type="setting" style="margin-right: 3px" theme="twoTone"  twoToneColor="#4a9ff5"  @click="edit(record)" v-hasPermission="'notice:edit'" title="修改"></a-icon>
        <a-icon v-if="record.status !== '3'" type="file-add" theme="twoTone"  style="color:#4a9ff5;margin:0" v-hasPermission="'notice:sound'" @click="soundClick(record)" title="发布" />
        <a-icon v-if="record.status == '3'" type="rollback"  style="color:#4a9ff5;"  v-hasPermission="'notice:repeal'" @click="repealClick(record)" title="撤销" />
      </template>
    </a-table>
  </div>
<!-- 修改 -->
...
export default {
  ...
  methods: {
    ...
    fach (parmse = {pageNum: 1, pageSize: 10}) {
      this.loading = true
      this.$get('/inform', parmse).then(res => {
    ...
    soundClick (record) {
      if (record.status !== 3) {
        ...
        that.$confirm({
          ...
          centered: true,
          onOk () {
            that.$get('/inform/release/' + record.id).then(res => {
              that.fach(that.character)
    ...
    repealClick (record) {
      if (record.status !== 2) {
        ...
        that.$confirm({
          ...
          centered: true,
          onOk () {
            that.$get('/inform/revoke/' + record.id).then(res => {
              that.fach(that.character)

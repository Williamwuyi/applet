      ...
      <div class="Share" v-for="(item,index) in arr" :key="index" @click.right="rightClick(item, $event)">
        ...
        <div v-else class="bgColor" style="padding-top: 13px;position: relative">
          <a href="#"  @dblclick="ClickShare(item)">
            ...
            <p :title="`创建时间： ${item.time}`" style="margin-top: 4px;text-align: center;color: #000c17;">{{item.oldName}}</p>
          </a>
        </div>
      </div>
      <!--    空文件提示    -->
...
<script>
import MyContentAdd from './MyContentAdd'
import MyContentEdit from './MyContentEdit'
import {notification} from 'ant-design-vue'
import axios from 'axios'
import store from '../../../store'
export default {
  name: 'Myminddle',
  components: {MyContentAdd, MyContentEdit},
  data () {
    return {
      Breadcrumb: [],
  ...
  methods: {
    ...
    onClick (key, index) {
      ...
      if (index === -1) {
        ...
        this.getFach({privateFileId: key.id})
    ...
    handleChange (info) {
      ...
      if (status === 'done') {
        ...
        this.$get('/publicFile/BindFile', parame).then(res => {
          this.getFach({privateFileId: this.parentId})
        })
    ...
    // 点击请求
    getFach (newD) {
      this.$get('/privateFile/getByIdList', newD).then(res => {
    ...
    handleDliele () {
      ...
      if (!that.current.fileId) {
        ...
        // } else {
        //   console.log(that.current.children.length)
        ...
        that.$confirm({
          ...
          centered: true,
          onOk () {
            that.$delete('/privateFile/' + that.current.id).then(() => {
              that.$notification.success({message: '系统提示', description: '操作成功！', duration: 4})
              that.getFach({privateFileId: that.parentId})
            })
          }
        })
      } else {

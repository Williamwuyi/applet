...
<script>
import SharedContentAdd from './SuperiorContentAdd'
import SharedContentEdit from './SuperiorContentEdit'
import {notification} from 'ant-design-vue'
import axios from 'axios'
import store from '../../../store'
export default {
  name: 'SuperiorMinddle',
  ...
  methods: {
    ...
    onClick (key, index) {
      ...
      if (index === -1) {
        ...
        this.getFach({manageFileId: key.id})
    ...
    handleChange (info) {
      ...
      if (status === 'done') {
        ...
        parame.manageFileId = this.parentId
        this.$get('/manageFile/BindFile', parame).then(res => {
          this.getFach({manageFileId: this.parentId})
        })
        ...
        that.$confirm({
          ...
          onOk () {
            that.$delete('/manageFile/' + that.current.id).then(() => {
              ...
              that.getFach({manageFileId: that.parentId})
            })
          }
        })
      } else {

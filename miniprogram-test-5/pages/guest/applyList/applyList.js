// pages/guest/applyList/applyList.js
...
Page({
  ...
  getVisitorList: function (status) {
    ...
    request.postData(domain, port,contentType, data, true).then(res => {
      // console.log('123', res.datas.records)
      // console.log('321',data.status)
      if (res.resp_code == 0) {
        if (data.status == 0) {
          this.setData({
  ...
  makeSummary: function (array) {
    ...
    for (var i = 0; i < array.length; i++) {
      currentSummary[i] = array[i].startTime + "去拜访" + array[i].person.personName
    }
    this.setData({
  ...
  navigateToDetailPage: function (e) {
    ...
    if (this.data.TabCur == 0) {
      ...
      this.takeValue(pickedVisitorId)

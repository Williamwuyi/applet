<template>
  <div>
    ...
    >
      ...
      <span slot="companies" slot-scope="comUnikey">
        <span v-for="(n,index) in allCompany" :key="index">
          <span v-if="comUnikey==n.comId">{{n.comName}}</span>
        </span>
      </span>
    </a-table>
    ...
    >
      <a-form :form="form">
        <a-form-item label="账号名" :labelCol="{span:8}" :wrapperCol="{span:14}">
          ...
          <a-input type="hidden" v-decorator="['isAdmin',{initialValue:1}]"></a-input>
        </a-form-item>
        ...
        <a-form-item label="所属公司" :labelCol="{span:8}" :wrapperCol="{span:14}">
          <a-select
            :disabled="isEidt"
            v-decorator="['comUnikey',{rules:[{required:true,message:'请选择公司'}]}]"
          >
            <a-select-option
              v-for="(n,index) in allCompany"
              :key="index"
              :value="n.comId"
...
export default {
  ...
  mounted() {
    ...
    axios
      ...
      .then(res => {
        this.allCompany = res.data.datas;
      });
  },
  methods: {
    //重载表格

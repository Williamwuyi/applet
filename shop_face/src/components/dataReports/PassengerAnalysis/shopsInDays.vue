  ...
  <div>
    ...
    <a-form layout="inline">
      <a-form-item label="起止日期">
        <a-date-picker :disabledDate="disabledStartDate" v-model="startTime" />
        <span style="font-size:18px;">~</span>
        <a-date-picker :disabledDate="disabledEndDate" v-model="endTime" />
      </a-form-item>
      <a-form-item>
...
const columns = [
  { title: "日期", width: "16%", dataIndex: "date", key: "1" },
...
export default {
  ...
  mounted() {
    ...
    this.startTime = this.endTime = moment(a, "YYYY-MM-DD");
  ...
  methods: {
    getDaysReport() {
      if (this.endTime == null || this.startTime == null) {
        this.$notification.warning({ message: "请选择起止时间" });
        return;
      }
      const params = {
        startDay: this.startTime.format("YYYY-MM-DD"),
        endDay: this.endTime.format("YYYY-MM-DD")
      };
      axios
        .get(
          this.root_path.rootIP +
            this.root_path.api6001 +
            "/eq/stat-customer/comStatByDaily",
          { params }
        )
        .then(res => {
    ...
    createCanvas(canvasid, name1, name2, cap1, cap2) {
      ...
      option = {
        ...
        xAxis: {
          name: "日期",
          type: "category",
          data: this.datalist.map(item => item.date.slice(-5))
        },
        yAxis: {
          name: "人数",
          type: "value"

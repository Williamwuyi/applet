  ...
  <div>
    ...
    <a-form layout="inline">
      ...
      <a-form-item label="指定周">
        <a-week-picker v-model="contrasDay" />
      </a-form-item>
      <a-form-item label="对比周">
        <a-week-picker v-model="day" />
      </a-form-item>
      <a-form-item>
    ...
    >
      <span slot="weeks" slot-scope="record">
        <span>{{record.date.slice(-5)+' ~ '+record.endDate.slice(-5)+'(第'+record.timeSlot+'周)'}}</span>
      </span>
    </a-table>
    <a-divider></a-divider>
...
export default {
  ...
  methods: {
    ...
    getMonday(data) {
      ...
      let l = d.getDate();
      d.setDate(l - a + 1);
      let xxx = moment(d, "YYYY-MM-DD");
      return xxx.format("YYYY-MM-DD");
    },
    // 切换tabs,展示新图表
    changeTabs(data) {
      if (data == 2) {
